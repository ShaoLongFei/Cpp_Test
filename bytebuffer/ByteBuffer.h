...
public:
    ...
    void getBytes(uint8_t *buf, uint32_t len) {
        ...
        for (uint32_t i = 0; i < len; i++) {
            cout<<"取出"<<p_buffer_[position_]<<endl;
...
private:
    ...
    void append(T data) {
        ...
        memcpy(&p_buffer_[limit_], (uint8_t *) &data, s);
        limit_ += s;
        cout<<"增加 "<< data << "当前状态 position_="<<position_<< " limit_="<<limit_<<endl;
    }
    ...
    void checkSize(uint32_t index, uint32_t increase) {
        ...
        uint32_t newSize = capacity_ + BUFFER_SIZE_INCREASE;
        uint8_t *pBuf = (uint8_t *) realloc(p_buffer_, newSize);
        cout << "重新分配内存 "<< newSize <<endl;
        if (!pBuf) {
...
private:
    const uint32_t BUFFER_SIZE_INCREASE = 10;
    std::string name_;
    uint8_t *p_buffer_;
    int32_t mark_;
    uint32_t limit_;
